...
每个条目都用 **`{YYYY-MM-DD} + {主标题简写}`** 作为跨文件锚点，方便 追溯模式 拼出时间线。
...
- `archived`：明确已结束/被取代——追溯模式 检索时降权但仍可查到
...
> 每条决策一个 ## section。每条带状态标记，便于 追溯模式 追溯历史拍板。
...
**重要**：被 `superseded` 的决策**不删**，是 追溯模式 追溯"我们之前为什么做了这个决定"的关键证据。

### 决策识别启发式（与 extraction-rules.md 一致）
...
> 每条待办一个 ## section。状态滚动更新，便于 追溯模式 追问"上次让我做的事完成没"。
...
- 追溯模式 被问到"上次让我做的事"时直接读这个文件，不依赖任何启发式
...
- **转写归档**：原 .txt 转写存放路径——便于 追溯模式 在需要原文时回头取证
...
月度小结是 boss-review skill 的产物，但 生成模式 也可以在每月第一次会议时**主动触发一次**回写。

---

## 6. boss_profile.md（老板偏好持久化，用户可编辑）

> 不属于"5 件套核心记忆"——这是**老板偏好配置文件**。第一次跑 生成模式 时一次性问完并生成，之后所有 skill 调用从这里读默认值。
>
...
- **文件不存在 + 调用方未传 `boss_name`** → 第一次跑 生成模式 时**一次性 batch 问** boss_name + boss_aliases + default_sharpness（其他字段用默认值），问完立即写文件
...
> 不属于"5 件套核心记忆"——这是 5 件套的**自动派生视图**。每次 生成模式 跑完会议（或老板手动触发刷新）时被**整体重写**。
>
> ⚠️ **不要手编**——下次刷新会全量覆盖。
...
*自动生成于 {YYYY-MM-DD HH:MM}，下次 生成模式 跑完会刷新。**不要手编。***
...
- 生成模式 Step 7.6（写完前 5 个文件后）**整体重写**（见 [../SKILL.md](../SKILL.md) A.2 Step 7.6）
- 老板手动触发：直接调 skill 传 `mode=refresh_dashboard`（占位约定，实现时再加）
...
当老板的查询**意图模糊到无法定位**到 6 类查询之一（例如"那事儿进展呢"），追溯模式**直接返回 boss-dashboard.md 的内容**作为默认答复，不再 AskUserQuestion 反问候选清单（详见 [../SKILL.md](../SKILL.md) B.3 Step 1）。
...
按查询意图加载（详见 [../SKILL.md](../SKILL.md) B 节）：
...
追溯模式 skill 的工作完全建立在这 5 个文件的格式约定上。**不要破坏字段名、不要改 ID 格式、不要把 markdown 改成 JSON**——契约一变两个 skill 都得改。
